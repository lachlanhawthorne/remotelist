...
For `remotelist-core`, the `build` command is the following:

```bash
tsup src/index.tsx --format esm,cjs --dts --external react
```

`tsup` compiles `src/index.tsx`, which exports all of the components in the design system, into both ES Modules and CommonJS formats as well as their TypeScript types. The `package.json` for `remotelist-core` then instructs the consumer to select the correct format:
...
Run `yarn build` to confirm compilation is working correctly. You should see a folder `remotelist-core/dist` which contains the compiled output.
...
Each file inside of `remotelist-core/src` is a component inside our design system. For example:

```tsx:remotelist-core/src/Button.tsx
...
```tsx:remotelist-core/src/index.tsx
import * as React from "react";
...
- Support using module path aliases like `@remotelist-core` for imports
- Write MDX for component documentation pages

For example, here's the included Story for our `Button` component:

```js:apps/docs/stories/button.stories.mdx
import { Button } from '@remotelist-core/src';
