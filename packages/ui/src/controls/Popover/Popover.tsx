...
export function Popover({ label, items, panelProps }: PopoverProps) {
...
function ContentItem(item: ItemProps) {
  const content = (
    <Fragment>
      <p tw="text-sm font-medium text-white">{item.name}</p>
  ...
  return (
    <a
      ...
      tw="
        ...
        hover:bg-gray-50 
        focus:outline-none 
        focus-visible:(ring ring-orange-500 ring-opacity-50)
      "
    >
      {Illustration ? (
        <Fragment>
    ...
    <HeadlessPopover.Button
      ...
      css={[
        tw`
          ...
          focus:outline-none 
          focus-visible:(ring ring-purple-500 ring-opacity-75)
        `,
        !open && tw`text-opacity-90`,
        open && tw`border-white`
      ]}
    >
      <span>{label}</span>
      <RiArrowDownSLine
